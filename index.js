...
bot.on('text', async (ctx, next) => {
    ...
    if (message.text.search('Игра окончена!') !== -1) {
        ...
        randomArray.map((item) =>
            gameMessages[item]
                ? ctx.reply(
                    `${gameMessages[item]?.text} -- ${gameMessages[item]?.author}`
                )
                : false
        );
    ...
    if (message.text.toLowerCase() === 'секс') {
        ctx.replyWithAnimation(
            'https://media1.tenor.com/m/Dr8YBE5s5AEAAAAC/sex-alarm.gif'
        );
    }
    if (
        message.text.toLowerCase().split('').slice(0, 10).join('') === 'бля ответь'
    ) {
        ctx.reply('ИДИ НАХУЙ');
    }
                ...
                .then((userProfilePhotos) => {
                    if (userProfilePhotos.total_count > 0) {
                        ...
                        bot.telegram.getFile(photo.file_id).then(async (fileInfo) => {
                            ...
                            const imageBuffer = await createImageWithTextAndPhoto(
                                photoUrl,
                                ctx.message.reply_to_message.text,
                                replyFrom.first_name +
                                ' ' +
                                (replyFrom.last_name?.length ? replyFrom.last_name : '')
                            );
...
bot.command('macat_siski', async (ctx) => {
    ...
    const topOneSize = playersArr.reduce(
        (accum, item) =>
            accum <= item.breastSize ? (accum = item.breastSize) : (accum = accum),
        0
    );
    const howFarAwayIs =
        Math.abs((topOneSize / playersArr[playerIndex]?.breastSize).toFixed(1)) ||
        1;
    ...
    if (playerIndex !== -1) {
        ...
        if (expiredTime < 14400000) {
            ctx.reply(
                `Друг (@${ctx.from.username}), не хуейте, вы сможете сыграть через ${14400000 - expiredTime
                }мс или ${Math.trunc((14400000 - expiredTime) / 60000)} минут`
            );
        } else {
            if (Math.trunc(Math.random() * 10) !== 0) {
                if (howFarAwayIs > 1.5) {
                    const bonusSize = Math.abs(
                        Math.trunc(
                            (playersArr[playerIndex].breastSize / 100) * 7 * howFarAwayIs
                        )
                    );
                    ctx.reply(
                        `Вы успешно дотянулись до своих tits @${ctx.from.username
                        } и помацали их, +${size}см и +${bonusSize} за отставание, теперь ваши сиськи размером ${playersArr[playerIndex].breastSize + size + bonusSize
                        }см, поздравляю друг`
                    );
                    ...
                    ctx.reply(
                        `Вы успешно дотянулись до своих tits @${ctx.from.username
                        } и помацали их, +${size}см, теперь ваши сиськи размером ${playersArr[playerIndex].breastSize + size
                        }см, поздравляю друг`
                    );
                ...
                ctx.reply(
                    `Вы немножко проебались @${ctx.from.username
                    }, -${size}см, теперь ваши сиськи размером ${playersArr[playerIndex].breastSize - size
                    }см, пиздуйте`
                );
                playersArr[playerIndex].breastSize -= size;
                playersArr[playerIndex].lastPlayedTime = dateNow;
        ...
        ctx.reply(
            `Добре пiжалавать в игру друг (@${ctx.from.username}), твои сиськи выросли на ${size} cм, зиг хайль!`
        );
    }
...
bot.command('post_meme', async (ctx) => {
    ...
    try {
        ...
        const response = await fetch(
            `https://api.imgbb.com/1/upload?key=19df7fadd3a92f9e74543ebb91637b87&image=${message}`
        );
...
async function createImageWithTextAndPhoto(photoUrl, text, quotedNickname) {
    const canvas = createCanvas(500, Math.trunc((text.length / 50) * 16) + 120);
    ...
    ctx.fillStyle = 'black';
    ctx.fillText(quotedNickname, 120, 30, 480);

    ctx.font = '16px Arial';
    ctx.fillStyle = 'black';
    wrapText(ctx, text, 120, 50, 460, 16);
